...
const ProductCategoryPortability: FC<IProductCategoryInterfaceProps> = ({
	setFilterDto,
	filterDto
}) => {
	...
	return (
		<FilterBlock title='Портативность'>
					...
					{array.map(i => (
						<div
							className={cn(
								...
								filterDto.portabilityCount! >= i && 'active'
							)}
							key={`display-${i}`}
							...
							onClick={() =>
								setFilterDto(prev => ({
									...prev,
									portabilityCount: prev.portabilityCount === i ? 0 : i
								}))
							}
						/>
					))}
...
export default ProductCategoryPortability
